...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	if (isset($_POST['AddCard'])) {
	    echo "<script>window.location = 'add_track.php';</script>";
	} elseif ($_POST['search']) {
		$search = true;
...
<?php
if ($search != false) {
	$sql = "SELECT * FROM track_list WHERE center_id = '".$_SESSION['center_id']."' AND name LIKE '%".$searchText."%' ORDER BY id ASC;";
} else {
	$sql = "SELECT * FROM track_list WHERE center_id = '".$_SESSION['center_id']."' ORDER BY id ASC;";
}
...
if ($_SESSION['group_name'] == "admin") {?>
        <div class="container">
        	<form id="form1" name="form1" method="post">
                <p align="center">
                  <label>
                  <input name="AddCard" type="submit" class="btn btn-success" id="AddCard" value="Add New Track">
                  </label>
