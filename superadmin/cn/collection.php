...
$customer_card_table = "CREATE TABLE IF NOT EXISTS collection (
...
$customer_expenses_table = "CREATE TABLE IF NOT EXISTS customer_expenses (
...
if(isset($_REQUEST['Search']) || isset($_POST['confirm']) || isset($_POST['cmdLock']))
{
	...
	{//echo $d_startDay ;
		...
		{
			$sql123 = "SELECT * FROM customer_expenses WHERE center_id = $t_center_id AND date = '".strtotime($d_date_disp)."' AND card_id =".$card_id;
			$result_expenses = mysqli_query($conn,$sql123);
			if(mysqli_num_rows($result_expenses) > 0)
			{
				$update_expenses = "UPDATE customer_expenses SET misc ='".$misc."',repair='".$repair."',advance='".$advance."',outbalance='".$out."',inbalance='".$in."' WHERE center_id = $t_center_id AND date = '".strtotime($d_date_disp)."' AND card_id=".$card_id;
				
				mysqli_query($conn,$update_expenses);
			}
			else
			{
				$insert_expenses = "INSERT INTO customer_expenses (`center_id`,`date`,`misc`,`repair`,`advance`,`outbalance`,`inbalance`,`card_id`) VALUES($t_center_id,'".strtotime($d_date_disp)."','".$misc."','".$repair."','".$advance."','".$out."','".$in."',".$card_id.")";
		...
		foreach($cross_check as $key => $cross_value)
		{
			if($cross_value == 1)
			{
				$sql_customer_card = "UPDATE customers_card SET crossed_date = '".date("d-m-Y")."', crossed = '1' WHERE id = ".$key;
		...
		foreach($clear_payment as $key => $clear_payment_value)
		{
			...
			{
				$sql_clear_payment = "UPDATE customers_card SET clear_payment = '1' WHERE id = ".$key;
		...
		foreach($collection as $key => $value)
		{
			...
			$sql_collection = "SELECT * FROM collection WHERE center_id = '$t_center_id' AND d_date_disp = '".strtotime($d_date_disp)."' AND card_id='$card_id'";
			$result_collection = mysqli_query($conn,$sql_collection);
			//echo $result_collection->num_rows.'/';
			if($result_collection->num_rows >0)
			{
				
				 $sql = "UPDATE collection SET amount=$amount,interest=$interest WHERE card_id = '$card_id' AND center_id = '$t_center_id' AND d_date_disp = '".strtotime($d_date_disp)."'";
...
<section>
  ...
  <table border="1" align="center" width="100%">
    <tbody><tr>
      <td>
	    <table cellspacing="0" cellpadding="0" border="0">
          <tbody><tr>
            <td colspan="3"><div align="left">中心编号 ID :<span class="SpecialHeader"><?php echo $t_center_id;?>
            ...
            <table bgcolor="#FF0000">
                <tbody>
                    ...
                    <td>      <input type="submit" name="cmdLock" value="开">
            ...
            <table bgcolor="#00FF00">
                <tbody>
                	<tr>
                        ...
                        <td><strong>开</strong></td>           
                        <td>      
                            <input type="submit" name="cmdLock" value="锁">                        </td>
                    </tr>				
               </tbody>
          	</table>
            <?php 
          ...
          <tr>
            <td colspan="3"><div align="left">日期 : <span class="SpecialHeader"><?php echo $d_startDay.'-'.$d_startMonth.'-'.$d_startYear; ?></span>
        ...
        <thead>
        	<tr>
                ...
                <th><div align="center">交叉</div></th>
                <th><div align="center">清除付款</div></th>
                <th><div align="center">数量</div></th>
                <th><div align="center">利息</div></th>
                <th><div align="center">名字</div></th>
                <th><div align="center">电话</div></th>
                <th><div align="center">Z</div></th>
                <th><div align="center">&#24320;&#22987;</div></th>
                <th><div align="center">nT</div></th>
                <th><div align="center">最后 </div></th>
                <th><div align="center">xT</div></th>
                <th><div align="center">DU</div></th>
                <th><div align="center">&#39038;&#23458; ID </div></th>
                <th><div align="center">卡号 ID </div></th>
          </tr>
	    ...
	    */
			...
			{
				$sql = "SELECT * FROM customers_card WHERE center_id = $t_center_id AND user_agent_id =".$agent_id." AND clear_payment != 1";
			}
		...
		if($result->num_rows > 0)
		{
	      ...
	      $sql2 = "SELECT * FROM `collection` WHERE d_date_disp = '".strtotime($selected_date)."' and center_id = $center_id and card_id = $card_id"; 
		...
		if($result->num_rows > 0)
		{
			...
			while($row = $result->fetch_assoc())
			{
	         ...
	         $sql2 = "SELECT * FROM `collection` WHERE d_date_disp = '".strtotime($selected_date)."' and center_id = $center_id and card_id = $card_id"; 
					...
					?>
                    <tr>
                        <td><a class="btn btn-success"  href="history.php?cust_cardid=<?php echo $card_id; ?>&amp;cardid=1" target="_self">历史</a></td>
        ...
        <tr>
        	<td colspan="15" class="ErrMsg"><div align="center">***** &#27809;&#26377;&#35813;&#20013;&#24515;&#30340;&#35760;&#24405;&#12290; &#35831;&#20877;&#27425;&#28155;&#21152;&#36873;&#25321;***** </div></td>
        </tr>
        <?php }?>
    </tbody></table>    </td>
    </tr>
    <?php  
		...
		if($_SESSION["group_name"]!='superadmin')			
		{
			$sql = "SELECT * FROM `customer_expenses` WHERE date = '".strtotime($selected_date)."' and center_id = $t_center_id and card_id IN(".implode(',',$customer_card).")"; 
		...
		else
		{
			...
			$sql = "SELECT * FROM `customer_expenses` WHERE date = '".strtotime($selected_date)."' and center_id = $center_id"; 
...
<?php }?>
    ...
    <tr>
      <td>        
		...
		<input name="confirm" class="btn btn-success" type="submit" id="confirm" value="确认并保存" />
		<?php }?>     </td>
    </tr>
  </tbody></table>
...
<form action="joblisting.php" method="post" name="form2" target="_self" id="form2">
  <div align="center">
    <input name="joblisting" type="submit" class="btn btn-success" id="joblisting" value="打印工作清单">
        ...
        <form id="form1" name="form1" method="get" action="collection.php">
            ...
            <div class="row form-group">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-2"></div>
                    <div class="col-md-6">
                    	<input name="Search" type="submit" class="btn btn-success" id="Search" value="搜寻">
