...
$sql = "CREATE TABLE IF NOT EXISTS recently_login
	(
...
if(isset($_POST['submit']))
{
	...
	if(empty($companyid))
	{
		...
		if(mysqli_num_rows($get_admin_data) > 0)
		{
			...
			<script>
			window.location.replace("<?php echo SITEURL?>/superadmin/dashboard.php");
			</script>
		<?php
		}
		else
	...
	else
	{
		...
		$sql= "SELECT * FROM super_admin_login WHERE companyid = '$companyid' AND password = '$Password' AND loginid = '$user_id' ";
		...
		if($result->num_rows>0)
		{
			...
			$_SESSION["superadmin_user_id"] = $row123['id'];
			//$_SESSION["agent_id"] = $row123['id'];
			//$user_id_table = $_SESSION["agent_id"] = $row123['id'];
			// $current_time = time();
			
			// // insert update 
			// $query_hwe = "SELECT * FROM recently_login where user_id=".$user_id_table;
			
			// 	$result_hwe1 = mysqli_query($conn, $query_hwe);
				
			// if(mysqli_num_rows($result_hwe1) > 0) {
			// 	$sql ="UPDATE recently_login SET date='$current_time' WHERE user_id = $user_id_table";
			// }
			// else{
			// 	$sql = "INSERT INTO recently_login (user_id, date)
