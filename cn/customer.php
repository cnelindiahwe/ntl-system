...
<div class="content-wrapper">
	...
	<a href="customer.php" class="btn btn-info">返回</a>
	<?php }
	if(isset($success))
	{
	?>
        	...
        	<table cellspacing="0" cellpadding="2" align="center" class="table table-bordered table-striped">
				  ...
				  ?>
                   ...
                   <tr>
                      ...
                      ?>
                      <td><?php printf("%0.2f", $subtotal);?></td>
                      <td>
                   	<?php
                   	if ($row['state'] == "Pending") {?>
                      	<a href="quotation.php?c=<?php echo $customer_id;?>&q=<?php echo $row['id']; ?>" class="btn btn-success">訂單詳情</a>
                      	<a onclick="return confirm('您確定取消此訂單？')" href="cancelled_quotation.php?q=<?php echo $row['id']; ?>" class="btn btn-success">取消訂單</a>
                      	<a href="confirm_quotation.php?q=<?php echo $row['id']; ?>" class="btn btn-success">確認訂單</a>
                   	<?php } elseif ($row['state'] == "Cancelled") {?>
                      	<a href="quotation.php?c=<?php echo $customer_id;?>&q=<?php echo $row['id']; ?>" class="btn btn-success">訂單詳情</a>
                   		<a onclick="return confirm('您確定恢復此訂單？')" href="cancelled_quotation.php?q=<?php echo $row['id']; ?>" class="btn btn-success">恢復訂單</a>
        ...
        <div class="container">
        	<form id="form1" name="form1" method="post" action="customer.php">
                <p align="center">
                  ...
                  <a href="quotation.php?c=<?php echo $_REQUEST['t_cust_id'];?>" class="btn btn-success" target="_self">添加新報價單</a>
                  </label>
                </p>
            </form>
        </div>
                            ...
                            <tr>
                            	<td>
                                	<a href="register_new_customer.php?cust=<?php echo $row['id'];?>" class="btn btn-success">編輯</a>
                                  	<a href="customer.php?t_cust_id=<?php echo $row['id'];?>&amp;Submit=1" target="_self" class="btn btn-success">卡</a>
                                </td>
                                <td><?php  echo ++$count;?></td>
                                <td><?php echo  $row['id'];?></td>
