...
<?php }?>
	...
	<table cellspacing="1" cellpadding="1" style="width:80%" class="table table-striped table-bordered" align="center">
		<caption class="menu-header">未收集报告</caption>
<?php
if ($_SESSION['center_id'] != 5) {?>
		<thead>
			<tr>
...
if ((isset($searchText) && $searchText != "") && (isset($searchDate) && $searchDate != "")) {
	$sql .= " AND (start_date LIKE '%".$searchDate."%' OR crossed_date LIKE '%".$searchDate."%' OR code LIKE '%".$searchText."%')";
} else if (isset($searchText) && $searchText != "") {
	$sql .= " AND (code LIKE '%".$searchText."%')";
} elseif (isset($searchDate) && $searchDate != "") {
	$sql .= " AND (start_date LIKE '%".$searchDate."%' OR crossed_date LIKE '%".$searchDate."%')";
}
...
<?php }?>
	...
	<table cellspacing="1" cellpadding="1" style="width:80%" class="table table-striped table-bordered show_center_card_list" align="center">
		<caption class="menu-header">顧客資料</caption>
		<thead>
			<tr>
				<th>Customer ID</th>
				<th>Name</th>
