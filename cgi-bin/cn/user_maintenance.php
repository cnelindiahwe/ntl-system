...
{
	$select_user = "SELECT * FROM users	WHERE id = ".$_REQUEST['uid'];
    	...
    	<div class="container">
            <form id="form1" name="form1" method="post" action="user_maintenance2.php">
            	...
            	<div class="row form-group">
                    ...
                    <div class="col-md-6">
                    	<input name="t_user_id" type="text" id="t_user_id" class="form-control" value="<?php echo $user_id;?>">
                    </div>
                    <div class="col-md-2"></div>
                </div> 
                <br/>   
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-6">
                    	<select name="t_group" id="t_group" class="form-control" style="width:50%">
                            <option value="" <?php if($group_name==''){ echo 'selected="selected"';}?>>--选择组--</option>
                            <option value="admin" <?php if($group_name=='admin'){ echo 'selected="selected"';}?>>行政</option>
                            <option value="center" <?php if($group_name=='center'){ echo 'selected="selected"';}?>>中心</option>
                        </select> 
                    </div>
                    <div class="col-md-2"></div>
                </div> 
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-6">
                    	...
                    	<input name="Newuser" class="btn btn-success" type="submit" id="Newuser" value="<?php if(isset($_REQUEST['uid'])){?>更新<?php }else {?>创造<?php }?>">&nbsp;&nbsp; <input type="reset" class="btn btn-success" name="Submit2" value="Reset">
    ...
    <section>
        ...
        <thead>
            <tr>
                ...
                <th><strong>用户身份  </strong></th>
                <th><strong>用户名 </strong></th>
                <th><strong>中心编号 </strong></th>
                <th><strong>用户群 </strong></th>
                <th>&nbsp;</th>
            </tr>
        </thead>
        ...
        <tbody>
			...
			if($_SESSION["group_name"]!='superadmin')
			{
				...
				$select_user = "SELECT * FROM users".$current_user_center;
				$result = mysqli_query($conn,$select_user);
					if($result->num_rows > 0)
					{
						$count=0;
						while($row = $result->fetch_assoc()){
							...
							$select_center_name = "SELECT center_name FROM center_detail WHERE ID = ".$row['center_id'];
							$center='';
            ...
            <tr>
                <td>
                	<a href="user_maintenance.php?uid=<?php echo $row['id'];?>&amp;cmdEdit=Edit" class="btn btn-success">编辑</a>
                </td>
                ...
                <td>
                    ...
                    <a href="change_password.php?id=<?php echo $row['id'];?>" class="btn btn-success">密码</a>
                </td>
            </tr>
            <?php }}?>
            ...
            <td colspan="7" class="ErrMsg">
                <div align="center">
                    <form id="form1" name="form1" method="post" action="user_maintenance.php">
                        <input name="cmdNew" type="submit" class="btn btn-success" id="cmdNew" value="新用户">
                    </form>
                </div>
            </td>
        </tr>
